...
public class IdealPageRank {
    ...
    public static void main(String[] args) {
        ...
        SparkSession spark = SparkSession.builder().appName("Java ideal page rank").getOrCreate();
        ...
        for (int i = 0; i < 15; ++i) {
            ...
            ranks = links.join(ranks).values().flatMapToPair(
                    s -> {
                        int urlCount = Iterables.size(s._1());
                        List<Tuple2<String, Double>> results = new ArrayList<>();
                        for (String url : s._1) {
            ...
            // Taxation: .mapValues(sum -> 0.15 + sum * 0.85);
        }
        ...
        }, false, 1).coalesce(1).saveAsTextFile("hdfs://boise:30221/PA3/results_ideal");
